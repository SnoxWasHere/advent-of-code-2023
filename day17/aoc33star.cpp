...
private:
    ...
    uint16_t h(const Coord& c) const {
        return 0*((bound - c.x - 1) + (bound - c.y - 1));
    }
    ...
    void insertOptions(const Traveler& in) {
        ...
        for (int i = 0; i < 3; i++) {
            if (pos == finishing) {pq.push({pos, score, 0, in.heading});}
            //lhs
            Coord resultant = pos + lhs;
            if (!outOfBounds(resultant)) {
                if (Traveler k = {resultant, uint16_t(score + at(resultant)), h(resultant), turn.first}; !(seen.contains(k.hash()))) {
                    pq.push(k);
                }
            }
            //rhs
            resultant = pos + rhs;
            if (!outOfBounds(resultant)) {
                if (Traveler k = {resultant, uint16_t(score + at(resultant)), h(resultant), turn.second}; !(seen.contains(k.hash()))) {
                    pq.push(k);
                }
            }
            pos = pos + card;
...
public:
    Grid(const array(uint8_t)& g) : vgrid(g), bound(g.size()) {
        pq.push({{0,1}, at({0,1}), h({0,1}), south});
        pq.push({{1,0}, at({1,0}), h({1,0}), east});
